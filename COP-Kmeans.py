...
def cop_kmeans(dataset, k, ml=[], cl=[], min_size=1, max_size=None, initialization='custom', max_iter=300, tol=1e-4):
    ...
    for _ in range(max_iter):
        ...
        for i, d in enumerate(dataset):
            ...
            if clusters_[i] == -1:
                ...
                while not found_cluster and counter < len(indices):
                    ...
                    if not violate_constraints(i, index, clusters_, ml, cl) and cluster_sizes[index] < max_size:
                        found_cluster = True
                        clusters_[i] = index
        ...
        clusters_, centers_ = compute_centers(clusters_, dataset, k, ml_info, min_size, max_size)
...
def initialize_centers(dataset, k, method):
    ...
    if method == 'custom':
        return dataset[[14, 22, 46, 1, 4, 21, 28, 20, 19, 33, 0, 11, 5, 13, 25, 78, 26, 2, 37, 9]]
...
def compute_centers(clusters, dataset, k, ml_info, min_size, max_size):
...
def get_ml_info(ml, dataset):
    ...
    scores = [sum(l2_distance(centroids[j], dataset[i]) for i in groups[j]) for j in range(len(groups))]
